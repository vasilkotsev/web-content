...
function scrollToLowerMenu() {
    if ($('.scroll-lowermenu').length) {

        //Click event to scroll to top
        $('.scroll-lowermenu').click(function() {
        	var target_lower_menu_id = $(this).attr('name');
        	$( '#'+target_lower_menu_id ).trigger( 'click' );
...
function selectUpperMenu() {
    if ($('.upper-menu-selector').length) {

        //Click event to scroll to top
        $('.upper-menu-selector').click(function() {
        	var target_upper_menu_id = $(this).attr('name');
        	var element = $( '#'+target_upper_menu_id );
        	var classattr = element.attr('class');
        	if ( classattr.indexOf("active-btn")==-1 ){
        		deactivateUpperMenus()
        		classattr = classattr+" "+"active-btn";
...
function deactivateOneUpperMenu(menuid){
	...
	if ( el!=null ){
		...
		classattr = classattr.replace(" active-btn", "");
		classattr = classattr.replace("active-btn ", "");
		el.attr("class",classattr);
	}
}

function clickLowerMenuTab(classname) {
    //$(classname).
	alert(classname);
}
